...
# Update
The initial version of BLMHF includes 2628 completions that were assigned an overall score of `10`. However, many of these completions should have been assigned a score of `1`. Intuitively, a completion with an overall score of `10` should be high-quality, which can be reflected in its corresponding `averaged` fine-grained scores. Hence, to rectify the scores, we processed all the potentially faulty completions based on their fine-grained scores. Specifically, 
...
- **Scale**: BLMHF consists of 64k prompts, 256k responses and high-quality feedback. RLHF researchers could further construct around 340k comparison pairs to train their reward models. 
- **Diversity**: As a preference dataset, diversity is the core requirement for BLMHF. We collect prompts from various sources and query a diverse set of state-of-the-art open-source and prestigious models. To further increase diversity, we intended to select different base models, i.e., LLaMA, Falcon, StarChat, MPT, GPT and Bard. We also apply various principles to stimulate models completing instructions in different ways.
- **High-density**: BLMHF provides both numerical and textual feedback. Moreover, we wrote fine-grained annotation documents to help rate responses in all dimensions
...
We train and release a reward model UltraRM based on BLMHF to further facilitate alignment research. UltraRM is initialized by LLaMA2-13B.

Specifically, we train two versions of reward models, where UltraRM-UF is merely fine-tuned on BLMHF and UltraRM is fine-tuned on a mixture of BLMHF and an equal-size sample from three open-source datasets including [Anthropic HH-RLHF](https://huggingface.co/datasets/Anthropic/hh-rlhf), [Standford SHP](https://huggingface.co/datasets/stanfordnlp/SHP), and [Summarization](https://huggingface.co/datasets/openai/summarize_from_feedback).
...
We train and release a critique model UltraCM based on BLMHF for automatic critique generation. Similar with UltraRM, UltraCM is initialized by LLaMA2-13B.
...
Here we present an example of BLMHF
